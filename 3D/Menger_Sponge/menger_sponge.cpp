...
// front
  ...
  program = LoadShaders( "../Shaders/menger.vert", "../Shaders/menger.frag" );
...
void onDisplay() {
  ...
  glm::mat4 view = glm::lookAt(glm::vec3(1.0, -4.5, 0.0), glm::vec3(0.0, 1.0, 0.0), glm::vec3(0.1, 1.0, 0.0));
  ...
  for (int i = 0 ; i < pyramid_curve.size() ; i++) {
    ...
    case 'F' : 
      ...
      model = glm::translate(model,glm::vec3(2.0,0.0,0.0)) ; break ;


    case 'f' :

      model = glm::translate(model,glm::vec3(2.0,0.0,0.0)) ; break ; // Translation
    ...
    case '-' :
      ...
      model = glm::rotate(model,-Pi_2,glm::vec3(0,0,1)); break ;
    ...
    case '(' :
      ...
      S_model.push(model) ; model = glm::scale(model,glm::vec3(1.0 / 3.0)) ; /*lambda = lambda/ 3. ; */ break ;
...
int main(int argc, char* argv[]) {
  ...
  LSystem hilbert("F","Ff()+-^&") ; 
  hilbert('F') = "(^f&f+F+FF+FF+FF+F&f^f+Ff+Ff+Ff+F&f^F+FF+FF+FF+F)f" ; 
  //hilbert('F') = "-ff+&ff^FFF+f+f-FF+f+f-F-f+f+f^fFF&f&f^F^^fff&+fFF&f&f^F^^fff&+fFF&f&f^F^^fff&+fFf&ff^++ff-fffff" ; 

  hilbert('f') = "f" ;
  /* "-ff+&ff^FFF+f+f-FF+f+f-F    
     -f+f+f^fFF&f&f^F^^fff&+fFF&f&f^F   
     ^^fff&+fFF&f&f^F^^fff&+fFf&ff^+  +ff-fffff"
